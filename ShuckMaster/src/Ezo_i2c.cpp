...
enum Ezo_board::errors Ezo_board::receive_read_cmd(){
	...
	char _sensordata[this->bufferlen]; //_sensordata is char array of length 32
	...
	if(this->error == SUCCESS){
		if(this->issued_read == false){
			...
			this->reading = atof(_sensordata); //This only takes the first value in _sensordata, but we want to get all of the data
      memcpy(this->data, _sensordata, sizeof data); //Here, we are copying the content in _sensordata to the data field 
      //get_data_buffer(_sensordata);
...
char Ezo_board:: get_last_received_reading(){  
  Serial.println(this->data);
  //Serial.print("String this->data[0:3]: ");
  //Serial.print(String(this->data[0]));
  //Serial.print(String(this->data[1]));
  //Serial.print(String(this->data[2]));
  //Serial.print(String(this->data[3]));
  //Serial.println();
...
enum Ezo_board::errors Ezo_board::receive_cmd( char * sensordata_buffer, uint8_t buffer_len) {
  ...
  //Serial.print(sensordata_buffer);
