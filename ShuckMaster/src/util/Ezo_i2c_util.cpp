...
void receive_and_print_reading(Ezo_board &Device) {              // function to decode the reading after the read command was issued
  ...
  //Maybe there is an intermediate step before we can receive the data???
  
  //char temp[32];
  //memcpy(temp, Device.get_last_received_reading(), sizeof temp);
  //Serial.println(temp);
  //Serial.println(String(temp));
  //Serial.println(String(Device.get_last_received_reading())); //Not printing the correct stuff :(
  //char tester[32]; 
  //memcpy(tester, Device.get_last_received_reading(), sizeof tester);
  //Serial.println(String(tester));
  ...
  //Serial.println(char(Device.get_last_received_reading()));
 //TODO: Be able to return the this->data as a char buffer so that we can easily 
 //      use the data for other stuff
}
